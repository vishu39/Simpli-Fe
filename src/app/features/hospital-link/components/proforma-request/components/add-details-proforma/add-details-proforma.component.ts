...
@Component({
  selector: "app-add-details-proforma",
  templateUrl: "./add-details-proforma.component.html",
  styleUrls: ["./add-details-proforma.component.scss"],
})
export class AddDetailsProformaComponent implements OnInit {
  ...
  buildForm() {
    this.proformaForm = this.fb.group({
      ...
      receivedAt: [""],
      piId: [this.requestData?.piId, [Validators.required]],
  ...
  submit() {
    if (this.proformaForm?.valid) {
      const { hospitalId, piId, patient, receivedAt } =
      ...
      this.faciliatorService
        .proformaInvoiceReceivedOpenLink(formData)
        .subscribe((res: any) => {
          this.refetch.emit();
          this.sharedService.showNotification("snackBar-success", res.message);
        });
    } else {
      this.proformaForm.markAsTouched();
    }
  }
}
