...
@Component({
  selector: "app-assign-proforma-email-fetch",
  templateUrl: "./assign-proforma-email-fetch.component.html",
  styleUrls: ["./assign-proforma-email-fetch.component.scss"],
})
export class AssignProformaEmailFetchComponent implements OnInit {
  @Input() patientId: any;
  ...
  getAllProformaInvoiceRequest() {
    this.facilitatorService
      .getAllProformaInvoiceRequest(this?.patientId)
      .subscribe((res: any) => {
  ...
  modifyPayload() {
    const { aggregator, patient, proformaInvoiceRequest, receivedAt } =
  ...
  onClickAggregator(item: any) {
    ...
    this.getAllProformaInvoiceRequestByAggregator(item);
  }

  getAllProformaInvoiceRequestByAggregator(item: any) {
    let payload = {
      patient: this.patientId,
      aggregator: item?._id,
    };
    this.facilitatorService
      .getAllProformaInvoiceRequestByAggregator(payload)
      .subscribe((res: any) => {
