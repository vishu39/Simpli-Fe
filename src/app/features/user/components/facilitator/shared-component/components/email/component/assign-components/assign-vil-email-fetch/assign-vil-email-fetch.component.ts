...
@Component({
  selector: "app-assign-vil-email-fetch",
  templateUrl: "./assign-vil-email-fetch.component.html",
  styleUrls: ["./assign-vil-email-fetch.component.scss"],
})
export class AssignVilEmailFetchComponent implements OnInit {
  @Input() patientId: any;
  ...
  vilObjFromAi: any;
  isAiLoading = true;
  getVilDataByEmailFetch() {
    this.isAiLoading = true;
    this.facilitatorService
      .getVilDataByEmailFetch(this.emailFetchData?._id)
      .subscribe(
        (res: any) => {
          this.vilDataFromAi = res?.data;
          if (this.vilDataFromAi?.attachment?.length > 0) {
            this.vilDataFromAi?.attachment?.map((file: any) => {
  ...
  patientData: any;
  getPatientById() {
    this.facilitatorService.getPatient(this.patientId).subscribe((res: any) => {
      ...
      // this.vilRequestForm.controls["patientName"].disable();
      // this.vilRequestForm.controls["treatment"].disable();
      // this.vilRequestForm.controls["country"].disable();
    });
  }
  ...
  getAllVilRequest() {
    this.facilitatorService
      .getAllVilRequest(this.patientId)
      .subscribe((res: any) => {
  ...
  deleteDonor(i: number) {
    ...
    if (indexArray?.length > 0) {
      indexArray?.forEach((ia: any) => {
        this.vilDataFromAi?.attachment.push(ia);
      });
    ...
    if (fileName === "fileFirst") {
      if (index !== -1) {
        if (!!this.fileFirstList[index]?.signedUrl) {
          this.vilDataFromAi?.attachment.push(this.fileFirstList?.[index]);
        }
    ...
    if (fileName === "fileSecond") {
      if (index !== -1) {
        if (!!this.fileSecondList?.[mainIndex]?.[index]?.signedUrl) {
          this.vilDataFromAi?.attachment.push(
  ...
  patchDraft() {
    let vilRequestDraftData: any = JSON.parse(
      localStorage.getItem(`${this.patientId}vilRequestDraft`)
    );
  ...
  saveDraft() {
    ...
    if (
      ...
      localStorage.setItem(
        `${this.patientId}vilRequestDraft`,
  ...
  onFileDrop(event: CdkDragDrop<any>, arrayIndex: number) {
    ...
    if (id === "fileFirst") {
      ...
      if (this.vilDataFromAi?.attachment?.length > 0) {
        let findIndex = this.vilDataFromAi?.attachment?.findIndex(
          (att: any) => att?.signedUrl === droppedData?.signedUrl
        );

        if (findIndex !== -1) {
          this.vilDataFromAi?.attachment?.splice(findIndex, 1);
        }
      }
    }
  ...
  onClickReadFile(data: any, type: string, index: any = null) {
    if (data?.uploadType === "fileUploaded") {
      ...
      if (data?.item?.type?.includes("image")) {
        this.facilitatorService
          .getPatientDataByEmailImageRead(this.emailFetchData?._id, payload)
          .subscribe((res: any) => {
            this.readFileData = res?.data;
            this.readFileNeededObj = this.readFileData?.patientData;
            this.isFileReading = true;
            this.fetchDataFromAi(this.readFileNeededObj, type, index);
        ...
        this.facilitatorService
          .getPatientDataByEmailFileRead(this.emailFetchData?._id, payload)
          .subscribe((res: any) => {
            this.readFileData = res?.data;
            this.readFileNeededObj = this.readFileData?.patientData;
            this.isFileReading = true;
      ...
      if (type === "patientPassport") {
        ...
        file = this.fileSecondList[index][data?.i];
      } else if (type === "donorPassport") {
        file = this.fileThirdList[index][data?.i];
      }
