...
import { AddOpinionComponent } from "../../../opinion/components/add-opinion/add-opinion.component";
...
export class AddPreIntemationComponent implements OnInit {
  ...
  submit() {
    if (this.preIntimationForm.valid) {
      let values = this.modifyPayload();
      this.faciliatorService.addPreIntimation(values).subscribe((res: any) => {
