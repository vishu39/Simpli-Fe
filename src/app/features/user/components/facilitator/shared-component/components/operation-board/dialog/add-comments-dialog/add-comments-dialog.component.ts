...
export class AddCommentsDialogComponent implements OnInit {
  ...
  readComment() {
    let data =
      this.data?.title === "Querry" || this.data?.title === "Comment"
        ...
        : this.data?.itemData;
    this.facilitatorService.readComment(data?._id).subscribe((res: any) => {
  ...
  getAllVilRequest() {
    ...
    this.facilitatorService
      .getPendingVilRequest(this.querryData?._id)
      .subscribe(
        (res: any) => {
          let data = res?.data;
  ...
  submitCloseFollowUp() {
    if (this.closeFollowUpFormGroup.valid) {
      ...
      this.facilitatorService.closeFollowUp(payload).subscribe((res: any) => {
        this.sharedService.showNotification("snackBar-success", res.message);
        this.subjectService.followUpApiRecallSubject.next(true);
        this.data.title = "";
        this.closeFollowUpFormGroup.reset();
