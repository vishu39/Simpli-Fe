...
@Component({
  selector: "shared-upcomming-arrival-list",
  templateUrl: "./upcomming-arrival-list.component.html",
  styleUrls: ["./upcomming-arrival-list.component.scss"],
})
export class UpcommingArrivalListComponent implements OnInit {
  actionButtons = [
  ...
  getQuerryData() {
    ...
    this.facilitatorService.getUpcommingQueryList(this.searchParams).subscribe(
      (res: any) => {
  ...
  downloadQuery() {
    this.facilitatorService.downloadUpcomingArrival().subscribe((res: any) => {
      let e = res?.data;
