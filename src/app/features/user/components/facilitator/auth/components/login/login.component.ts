...
export class LoginComponent implements OnInit {
  ...
  constructor(
    ...
    localStorage.setItem("loginType", "facilitator");
    this.sharedService.facilitatorLoginSubject.next("admin");
  ...
  selectUser(userType: string) {
    if (userType === "admin") {
      ...
      this.isMember = true;
      this.isAdmin = false;
      this.sharedService.facilitatorLoginSubject.next("member");
  ...
  //           this.router.navigate(["/user/facilitator/admin"]);
  //           localStorage.setItem("loginType", "facilitator");
  //           this.sharedService.facilitatorLoginSubject.next("admin");
  //           this.sharedService.showNotification(
  //             "snackBar-success",
  ...
  //           localStorage.setItem("userToken", res.data.userToken);
  //           localStorage.setItem("cmsToken", res.data.cmsToken);
  //           this.router.navigate(["/user/facilitator/internal-user"]);
  //           localStorage.setItem("loginType", "facilitator");
  //           this.sharedService.facilitatorLoginSubject.next("member");
  //           this.sharedService.showNotification(
  //             "snackBar-success",
  ...
  verifyOtp() {
    if (this.otpControl.valid) {
      ...
      this.facilitatorService.verifyOtp(payload).subscribe((res: any) => {
  ...
  redirectToHospitalAuth() {
    this.router.navigate(["user/hospital/auth/login"]);
  }
}
