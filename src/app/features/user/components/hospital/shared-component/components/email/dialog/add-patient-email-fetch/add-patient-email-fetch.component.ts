...
@Component({
  selector: "app-add-patient-email-fetch",
  templateUrl: "./add-patient-email-fetch.component.html",
  styleUrls: ["./add-patient-email-fetch.component.scss"],
})
export class AddPatientEmailFetchComponent implements OnInit {
  @Input() emailData: any;
  ...
  ngOnInit(): void {
    this.getPatientDataByEmailFetch();
    this.getCountryData();
  ...
  isEmailLoading = true;
  getPatientDataByEmailFetch() {
    this.isEmailLoading = true;
    this.hospitalService
      .getPatientDataByEmailFetch(this.emailData?._id)
      .subscribe(
        (res: any) => {
  ...
  onEdit(data: any) {
    ...
    if (this.commingEmailData?.attachment?.length > 0) {
      this.commingEmailData?.attachment?.forEach((res: any) => {
  ...
  onClickReadFile(data: any) {
    ...
    if (data?.item?.type?.includes("image")) {
      this.hospitalService
        .getPatientDataByEmailImageRead(this.emailData?._id, payload)
        .subscribe((res: any) => {
      ...
      this.hospitalService
        .getPatientDataByEmailFileRead(this.emailData?._id, payload)
        .subscribe((res: any) => {
          this.readFileData = res?.data;
          this.readFileNeededObj = this.readFileData?.patientData;
          this.isFileReading = true;
