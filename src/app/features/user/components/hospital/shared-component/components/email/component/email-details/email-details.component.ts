...
import { ReplyToAllComponent } from "../reply-to-all/reply-to-all.component";
...
export class EmailDetailsComponent implements OnInit {
  ...
  getEmailFetch() {
    ...
    this.hospitalService.getEmailFetch(this.id).subscribe(
      (res: any) => {
...
<div><strong>subject:</strong> ${this.emailData?.subject || "NIL"}</div>
  ...
  openAddPatientModal() {
    ...
    // const dialogRef = this.dialog.open(AddPatientEmailFetchComponent, {
    //   width: "100%",
    //   disableClose: true,
    //   autoFocus: false,
    // });
    // dialogRef.componentInstance.dialogTitle = "Add Patient";
    // dialogRef.componentInstance.emailData = this.emailData;
