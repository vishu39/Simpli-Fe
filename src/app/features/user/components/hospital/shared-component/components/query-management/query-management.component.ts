...
import { ForwardToDoctorComponent } from "../treating-doc-query-management/components/forward-to-doctor/forward-to-doctor.component";
...
@Component({
  selector: "app-shared-query-management",
...
export class QueryManagementComponent implements OnInit {
  ...
  queryEventHandler(data: any) {
    ...
    if (data?.mode === "Forward to doctor") {
      this.forwardToDoctor();
    }
  }

  forwardToDoctor() {
    const dialogRef = this.dialog.open(ForwardToDoctorComponent, {
      width: "100%",
      disableClose: true,
      autoFocus: false,
    });
    dialogRef.componentInstance.dialogTitle = "Forward To Doctor";
    dialogRef.componentInstance.patient = this.queryData;
    ...
    this.svc.ui
      ...
      .subscribe((res) => {
        if (res.button.name === "YES") {
          this.hospitalService.deletePatient(id).subscribe((res: any) => {
