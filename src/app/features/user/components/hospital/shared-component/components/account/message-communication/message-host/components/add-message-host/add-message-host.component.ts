...
@Component({
  selector: "app-add-message-host",
  templateUrl: "./add-message-host.component.html",
  styleUrls: ["./add-message-host.component.scss"],
})
export class AddMessageHostComponent implements OnInit {
  dialogTitle: string;
  ...
  constructor(
    private dialogRef: MatDialogRef<AddMessageHostComponent>,
  ...
  submit() {
    if (this.addEmailFetchSettingForm.valid) {
      ...
      if (!this.isEdit) {
        this.hospitalService.addMessageSetting(values).subscribe((res: any) => {
        ...
        this.hospitalService
          .editMessageSetting(values, this.emailFetchData?._id)
          .subscribe((res: any) => {
