...
export class MessageFetchAssignVilComponent implements OnInit {
  ...
  searchEmbassy(filterValue: string) {
    ...
    this.timeoutEmbassy = setTimeout(() => {
      if (!!filterValue) {
        let filterArray: any = cloneDeep(this.embassyFreshData);
        this.embasyData = [];
        let filterData = filterArray.filter((f: any) =>
  ...
  patchDraft() {
    ...
    if (!!vilRequestDraftData) {
      let newData: any = cloneDeep(vilRequestDraftData);
  ...
  saveDraft() {
    ...
    let newAttendantDetails: any = cloneDeep(formValue?.attendantDetails);
    let newDonorDetails: any = cloneDeep(formValue?.donorDetails);
  ...
  onClickReadFile(data: any, type: string, i: any = null) {
    if (data?.uploadType === "fileUploaded") {
      let payload = {
        url: data?.item?.url || data?.item?.path || data?.item?.signedUrl,
