...
@Component({
  selector: "app-add-opd-email-fetch",
  templateUrl: "./add-opd-email-fetch.component.html",
  styleUrls: ["./add-opd-email-fetch.component.scss"],
})
export class AddOpdEmailFetchComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.buildForm();
    this.getAddOpdDataByEmailFetch();
  ...
  addOpdObjFromAi: any;
  isAiLoading = true;
  getAddOpdDataByEmailFetch() {
    this.isAiLoading = true;
    this.hospitalService
      .getAddOpdDataByEmailFetch(this.emailFetchData?._id)
      .subscribe(
        (res: any) => {
  ...
  patchDraft() {
    let opdDraftData: any = JSON.parse(
      localStorage.getItem(`${this.patientId}addOpdDraft`)
    );
    if (!!opdDraftData) {
      if (
    ...
    // localStorage.removeItem(`${this.patientId}addOpdDraft`);
  }
  ...
  getPendingOpdRequest() {
    ...
    this.hospitalService
      .getPendingOpdRequest(this.patientId)
      .subscribe((res: any) => {
