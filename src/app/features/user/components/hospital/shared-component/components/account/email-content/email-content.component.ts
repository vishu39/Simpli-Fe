...
@Component({
  selector: "shared-hospital-email-content",
...
export class EmailContentComponent implements OnInit {
  ...
  getAllDefaultEmailContent() {
    ...
    this.hospitalService.getAllDefaultEmailContent().subscribe(
      (res: any) => {
    ...
    this.svc.ui
      ...
      .subscribe((res) => {
        if (res.button.name === "YES") {
          this.hospitalService.deleteEmailContent(id).subscribe((res: any) => {
